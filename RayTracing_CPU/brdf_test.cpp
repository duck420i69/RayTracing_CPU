...
#if 0
vec2 toSphericalCoordinate(vec3 v) {
...
vec3 toCartesianCoordinate(vec2 v) {
    if (v.x >= pi) v.x -= pi;
...
void test_material(std::shared_ptr<Material>& material) {    
    Packet packet;
    ...
    size_t h = 100;
    size_t w = 100;
    std::vector<std::vector<SampleBin>> sampleBins = std::vector<std::vector<SampleBin>>(100, std::vector<SampleBin>(100, {}));
...
int main() {
    ...
    for (int i = 0; i < 1; i++) {
        std::cout << material->name + " test " << i + 1 << std::endl;
